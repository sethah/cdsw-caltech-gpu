...
def image_encode(args, i, item, q_out):
    ...
    if args.resize and args.newsize is None:
        if img.shape[0] > img.shape[1]:
            newsize = (int(args.resize), int(img.shape[0] * args.resize / img.shape[1]))
        else:
            newsize = (int(img.shape[1] * args.resize / img.shape[0]), int(args.resize))
        img = cv2.resize(img, newsize)
    elif args.newsize is not None:
       left, right = args.newsize.split(",")
       left = int(left)
       right = int(right)
       img = cv2.resize(img, (left, right))
...
def parse_args():
    ...
    rgroup.add_argument('--newsize', type=str, default=None,
                        help='resize square')
